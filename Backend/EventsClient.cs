...
namespace Backend {
    public class EventsClient : IEventsClient {
        ...
        public async Task Client_ClientError(DiscordClient sender, ClientErrorEventArgs clientErrorEventArgs) {
            _logger.Log($"Exception occurred: {clientErrorEventArgs.Exception.GetType()}: {clientErrorEventArgs.Exception.Message}", LogLevel.Error);
            await Task.CompletedTask;
        }
        ...
        public async Task Commands_CommandExecuted(CommandsNextExtension commandsNextExtension, CommandExecutionEventArgs commandExecutionEventArgs) {
            _logger.Log($"{commandExecutionEventArgs.Context.User.Username} successfully executed '{commandExecutionEventArgs.Command.QualifiedName}'", LogLevel.Information);
            await Task.CompletedTask;
        }
