...
namespace Luigis_Pizza.Commands
{
    public class Core : BaseCommandModule
    {
        ...
        [Command("toggletimer"), Aliases("-tt"),
         Description(
             "This command enables/disables the timer. It also uses interactity with a configured timespan of 15 seconds.")]
        public async Task ToggleTimer(CommandContext ctx)
        {
            var interActExt = ctx.Client.GetInteractivity();

            await ctx.RespondAsync(
                $"With this command you can enable `(true)` or disable `(false)` the timer. Current status is `{EnableTimer}`.");
            ...
            else
            {
                ...
                {
                    await ctx.RespondAsync(
                        "The input is invalid. Run the command again and use either `true` to enable or `false` to disable the timer.");
                }
            }
        }

        [Command("settime"), Aliases("-st"),
         Description(
             "This command sets a new timespan for the timer. It also uses interactity with a configured timespan of 15 seconds.")]
        public async Task SetTimeSpan(CommandContext ctx)
        {
            ...
            else
            {
                TimerSpan = 10000d;
                await ctx.RespondAsync(
                    $"The input is invalid. Setting the timerspan to its default value: {TimerSpan}");
            }
        }
    }
}