...
namespace Luigis_Pizza.Commands
{
    ...
    {
        public static double TimerSpan = 10000d;
        ...
        public async Task ToggleTimer(CommandContext ctx)
        {
            ...
            // Store the reply of the user
            var Message = await InterActExt.WaitForMessageAsync(xm => xm.Author.Id == ctx.User.Id, TimeSpan.FromSeconds(15));
        ...
        public async Task SetTimeSpan(CommandContext ctx)
        {
            ...
            // Check if the input can be converted to double
